...
import org.springframework.coap.server.accept.HeaderContentTypeResolver;
import org.springframework.coap.server.accept.RequestedContentTypeResolver;
import org.springframework.coap.server.accept.RequestedContentTypeResolverBuilder;
import org.springframework.lang.Nullable;

public class CoapProducesRequestCondition extends AbstractRequestCondition<CoapProducesRequestCondition> {
	 ...
	 * @param produces expressions with syntax defined by {@link CoapRequestMapping#produces()}
	 */
	public CoapProducesRequestCondition(String... produces) {
		this(produces, null);
	}
	 ...
	 * @param produces expressions with syntax defined by {@link CoapRequestMapping#produces()}
	 * @param headers expressions with syntax defined by {@link CoapRequestMapping#headers()}
	 */
	public CoapProducesRequestCondition(String[] produces, String[] headers) {
		this(produces, headers, null);
	}

	/**
	 * Same as {@link #CoapProducesRequestCondition(String[], String[])} but also
	 * accepting a {@link RequestedContentTypeResolver}.
	 * @param produces expressions with syntax defined by {@link CoapRequestMapping#produces()}
	 * @param headers expressions with syntax defined by {@link CoapRequestMapping#headers()}
	 * @param resolver used to determine requested content type
	 */
	public CoapProducesRequestCondition(String[] produces, String[] headers, RequestedContentTypeResolver resolver) {
	 ...
	 */
	private CoapProducesRequestCondition(Collection<ProduceMediaTypeExpression> expressions,
	 ...
	 */
	@Override
	public CoapProducesRequestCondition combine(CoapProducesRequestCondition other) {
	...
	@Override
	@Nullable
	public CoapProducesRequestCondition getMatchingCondition(ServerCoapExchange exchange) {
		if (isEmpty()) {
			return this;
		}
		...
		return (result.isEmpty()) ? null : new CoapProducesRequestCondition(result, this.contentTypeResolver);
	}
	 ...
	 */
	@Override
	public int compareTo(CoapProducesRequestCondition other, ServerCoapExchange exchange) {
		try {
	...
	private List<MediaType> getAcceptedMediaTypes(ServerCoapExchange exchange)
	...
	private int compareMatchingMediaTypes(CoapProducesRequestCondition condition1, int index1,
			CoapProducesRequestCondition condition2, int index2) {
	...
	class ProduceMediaTypeExpression extends AbstractMediaTypeExpression {
		...
		@Override
		protected boolean matchMediaType(ServerCoapExchange exchange) throws NotAcceptableStatusException {
			List<MediaType> acceptedMediaTypes = getAcceptedMediaTypes(exchange);
			for (MediaType acceptedMediaType : acceptedMediaTypes) {
